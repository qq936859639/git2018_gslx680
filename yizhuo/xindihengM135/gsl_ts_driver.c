...
static void gsl_report_work(void)
{
	...
	//u8 i = 0;
	//u16 ret = 0;
#if defined(DROI_PRO_PU6A_DNT_P09)||defined(DROI_PRO_PWU5_XDH_M135M)
	char point_num = 0;
#endif
#ifdef TPD_PROXIMITY
	...
	if(tmp1>0&&tmp1<0xffffffff)
	{
		...
		if(GE_ENABLE == gsl_gesture_status && gsl_gesture_flag == 1){
			...
			if(key_data != 0){
				...
				//gsl_gesture_status = GE_WAKEUP; //zxw modify for mx1091_hyf_9300 \CFȻ\AE\CE\DEЧ\B5\C4\CA\D6\CA\C6Ȼ\BA\F3\BB\AE\D3\D0Ч\B5\C4\CA\D6\CAƲ\BB\C6\F0\D7\F7\D3\C3
...
#ifdef GSL_GESTURE
		input_set_capability(tpd->dev, EV_KEY, KEY_POWER);//\A1\C1\A1\E92\A8\A2\A8\BA?\A8\A8?\A1\C1\A8\AE?\A6?\AA3
