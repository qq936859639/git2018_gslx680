...
#ifdef GTP_SUPPORT_I2C_DMA
	u8 *g_gsl_dma_buff_va = NULL;
	dma_addr_t g_gsl_dma_buff_pa = 0;
#endif
...
static int tpd_keys_dim_local[TPD_KEY_COUNT][4] = TPD_KEYS_DIM;
	...
	{
	    if (NULL == g_gsl_dma_buff_va)
    		{
       		 tpd->dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
       		 g_gsl_dma_buff_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, 128, &g_gsl_dma_buff_pa, GFP_KERNEL);
    		}

	    	if(!g_gsl_dma_buff_va)
		{
	...
	static void msg_dma_release(void){
		if(g_gsl_dma_buff_va)
		{
	     		dma_free_coherent(NULL, 128, g_gsl_dma_buff_va, g_gsl_dma_buff_pa);
	        	g_gsl_dma_buff_va = NULL;
	        	g_gsl_dma_buff_pa = 0;
        ...
        u8 reg, u8 *buf, u32 num)
{
	...
	if(temp[0] < 0x80)
	{
		...
		memcpy(g_gsl_dma_buff_va, temp, 1);

		i2c_master_send(client,(unsigned char *)g_gsl_dma_buff_pa,1);	
		err = i2c_master_recv(client,(unsigned char *)g_gsl_dma_buff_pa,4);
	...
	{	
		...
		i2c_master_send(client,(unsigned char *)g_gsl_dma_buff_pa,1);
		if((i+8)<num)
			err = i2c_master_recv(client,((unsigned char *)g_gsl_dma_buff_pa+i),8);
		else
			err = i2c_master_recv(client,((unsigned char *)g_gsl_dma_buff_pa+i),(num-i));
		i+=8;
	}
	}

	memcpy(buf, g_gsl_dma_buff_va, num);
...
s32 gsl_write_interface(struct i2c_client *client, u8 reg, u8 *buf, u32 num)
{
	...
	else if( 1 &&(NULL != g_gsl_dma_buff_va))
	{
		for (i = 0; i < writelen; i++)
		{
			g_gsl_dma_buff_va[i] = wr_buf[i];
			//print_info("[DMA][gsl_write_interface] g_gsl_dma_buff_va[%d]=%x\n",i,g_gsl_dma_buff_va[i]);
		}

	    client->addr = (client->addr & I2C_MASK_FLAG )| I2C_DMA_FLAG;
	    ret = i2c_master_send(client, (unsigned char *)g_gsl_dma_buff_pa, writelen);
	    client->addr = client->addr & I2C_MASK_FLAG & ~I2C_DMA_FLAG;
...
static int  gsl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{	
		print_info("==gslX680111111111111111111111-----------erro----------addr=0x%x\n",ddata->client->addr);
	...
	if(err<0)
	{
		print_info("[tpd-gsl][%s] err!!! gsl_test_i2c=%d\n",__func__,err);
		goto  err_malloc;
	}
